...
def fem_matrix(_x, _y, _numele, _numnode, _ien):
    ...
    gx3 = sp.zeros((_numnode, _numnode), dtype="float64")
...
for i in range(nodes_fluid):
    ...
    # psi_a[i] = -0.25 * dpdx * (0.5 * y_fluid[i] - 0.25 * y_fluid[i]**3)
    psi_a[i] = -0.25 * dpdx * (0.5 - 0.25 * y_fluid[i]**2)
...
# vz = sp.multiply(MinvLump, sp.dot(M3, psi_a)) + sp.multiply(MinvLump, sp.dot(Gy, psi_a))

result = sp.zeros(nodes_fluid)
for i in range(nodes_fluid):
    result[i] = psi_a[i] / (y_fluid[i] + 0.001)

# vz = 2*psi_a + sp.multiply(MinvLump, sp.dot(Gy, psi_a))
vz = 2*psi_a + sp.multiply(MinvLump, sp.multiply(y_fluid, sp.dot(Gy, psi_a)))

# vz = sp.multiply(MinvLump, sp.dot(Gy2, y_fluid**2 * psi_a))
vr = -1.0 * sp.multiply(MinvLump, sp.dot(Gx, sp.multiply(y_fluid, psi_a)))
...
vtk_t.saveVTK(cwd+"/resultsTest", mesh_file + "Test-h")
