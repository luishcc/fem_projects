...
malha_fldH = Gm.GMesh(arquivo+"-fld.msh")
...
for i in range(len(malha_fldH.dirichlet_points)):
    ...
    if y_fldH[index] != 1.0 and y_fldH[index] != 0.5 and y_fldH[index] != 0.0:
...
lista = []
for i in range(nodes_fldH):
    if (x_fldH[i] == 0.0 or x_fldH[i] == 2.0) and (y_fldH[i] == 0.5 or y_fldH[i] == 1.0):
...
neighbour_fld = sl.neighbourElements(nodes_fldH, ien_fldH)
neighbour = sl.neighbourElements(nodes_total, ien_total)
...
for t in range(0, tempo):
    ...
    fw_last = sl.Linear2D(nodes_fldH, neighbour_fld, ien_fldH, x_fldH, y_fldH, vx_h, vy_h, dt, Wz_old_h)

    F_omega_h = sp.dot(Mdt_fld_h, fw_last) + ccomega_h
    for i in range(num_omega_h):
        index = int(Fluid_Boundary_h[i])
    ...
    ft_last = sl.Linear2D(nodes_total, neighbour, ien_total, x_total, y_total, vx_total, vy_total, dt, Temp_old)
    F_temp = sp.dot(Mdt, ft_last) + cctemp
    for i in range(dirichlet_len_total):
        index = int(malha_total.dirichlet_points[i][0]) - 1
